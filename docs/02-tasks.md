...
The estimates below total **~26 hours against a 2–3 day budget.** That is a real
...
```
T1 → T3 → T5 → T7 → T8 → T15 → T16 → T19
```

≈ 13 h. Everything else is slack around this spine. T15 (brownfield) cannot
start until the redirect path is real, and T16 (ambiguous, privacy) depends on
click recording existing to make privacy-safe — so the spine must be protected
early. Slipping T5 or T7 by half a day compresses both scenarios directly.

## Phase A — Foundation

| ID | Task | Depends on | Est | Done when |
| --- | --- | --- | --- | --- |
| **T1** | Bootstrap: Maven, Java 21, Spring Boot 3.x, package layout, `docker-compose.yml` with PostgreSQL 16 | — | 1.5 h | `mvn verify` passes and the app starts against Compose PostgreSQL |
| **T2** | Quality gates in the build: Spotless, Checkstyle, SpotBugs, OWASP Dependency-Check | T1 | 1.0 h | All four run in `mvn verify` and fail the build on violation |
...
| **T4** | Short-code generator (base62 / `SecureRandom`) + unit tests | T1 | 1.5 h | Alphabet and length asserted; generator is an interface so it can be stubbed in T5's collision test |
...
| ID | Task | Depends on | Est | Done when |
| --- | --- | --- | --- | --- |
| **T5** | Persistence: Spring Data JDBC repositories + Testcontainers integration tests, incl. **collision-retry** and **concurrent-creation** tests | T3, T4 | 1.5 h | Retry path proven by a forced collision; parallel creates all yield distinct codes |
| **T6** | `POST /api/links` — creation, URL validation, scheme allowlist, private-address rejection, length cap | T5 | 1.5 h | Valid URL returns 201 + code; each rejection class returns 400 with a problem body |
...
| ID | Task | Depends on | Est | Done when |
| --- | --- | --- | --- | --- |
| **T15** | **Brownfield: link expiration.** `V2` migration adding nullable `expires_at`; expiry check on resolve returning 410; `POST /api/links` accepts optional TTL; tests; impact analysis written to `docs/scenario-brownfield.md` | T7, T9 | 2.5 h | Existing rows unaffected (backward-compatible migration proven by test); expired link returns 410; impact analysis covers API, service, schema, tests, docs |
| **T16** | **Ambiguous: "make analytics privacy-safe."** Interpretation, alternatives and chosen policy; **expand/contract** `V3`+`V4` (stop writing `client_ip`, then drop it), referrer reduced to host, user-agent bucketed; retention policy; written to `docs/scenario-ambiguous.md` | T8, T15 | 2.75 h | A2/A3 debt in `01-decisions.md` is discharged; the column drop is ordered *after* the code that stops writing it, per the migration policy in `03-architecture.md`; the reasoning from ambiguity to policy is legible to a reviewer |
...
| **T19** | Final engineering summary: plan, decisions, artifacts, validation, risks, trade-offs, limitations, next steps | all | 1.5 h | Assignment §5 deliverables each map to a committed path |
| **T20** | AI traceability log — **continuous**, committed alongside the code it describes | all | ~1.0 h aggregate | Entries exist for every task that used AI, including rejections |
...
That is **~2.75 h of genuine slack against a ~26 h estimate.** Thin, and stated
...
- **T21** — a quality gate that runs only on the engineer's machine is not a
  gate, and the README claims four of them.

## Risks

| Risk | Mitigation |
| --- | --- |
| Testcontainers startup cost compounds across test classes | Single static container on a shared abstract base class, wired with `@ServiceConnection`. Budgeted in T5. Rejected an H2 fallback — see T6 in `01-decisions.md` |
...
| T16's column drop is destructive and Flyway Community has no undo | Expand/contract: `V3` stops the write, `V4` drops the column. Forward-only policy stated in `03-architecture.md`; the drop is irreversible by design, which is the point of a privacy change, but it is a deliberate choice rather than an accident |
